...
model_eval = LogisticRegression(class_weight='balanced', penalty='l1', C=0.1, solver='liblinear')
model_eval.fit(X, y)
# print(eli5.__version__)
eli5.explain_weights(model_eval)
print((model_eval.coef_ != 0).sum())
print(eli5.formatters.as_dataframe.explain_weights_df(model_eval).feature)
top_features = [i[1:] for i in eli5.formatters.as_dataframe.explain_weights_df(model_eval).feature if 'BIAS' not in i]
print(top_features)
...
file = pd.DataFrame({"id":np.arange(X_test.shape[0])+250, "target":y_pred})
file.to_csv("dont-overfit-ii/data/shokhmi4.csv", index=False)
